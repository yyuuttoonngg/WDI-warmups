...
function startupGenerator(){
    var ranNum1 = Math.floor(Math.random()*thisArray.length);
    var ranNum2 = Math.floor(Math.random()*thatArray.length);
    var combinedName = thisArray[ranNum1] + ' for ' + thatArray[ranNum2];
    //console.log(combinedName);
    nameArray.push(combinedName);

};

for (i=1; i<6; i++){
    startupGenerator();
    console.log( i + '. '+ nameArray[i-1]);
    document.getElementById('nameIdeas').innerHTML += i + '. '+ nameArray[i-1] + ' <br/>'; 
};
